...
//     future and past year -- date_range=prior12
	...
	function __construct( &$userAttorneyId, &$addOnCaseNumbers, &$addOnBarNumbers, $verbose = true ) {
        ...
        $this->userSchedule = self::initializeBarNumSchedule( $userAttorneyId );
        ...
        if ( !empty( $addOnBarNumbers ) ) {
            foreach( $this->addOnBarNumbers as $barNumber ) {
                ...
                $this->addOnBarNumberSchedule = array_merge(
                    $addOnBarNumberSchedule, self::initializeBarNumSchedule( $barNumber ) );
            }
	...
	protected function initializeBarNumSchedule( &$barNumber ){
	    if ( $this->verbose ) echo  __METHOD__ . "\n";
	    // check if db data is stale
	    if ( self::isScheduleStale( $barNumber ) ) {
	        ...
	        $tempNACs = self::parseNACTables( $htmlStr );
            // self::insertNacs ( $tempNACs );
            return $tempNACs;
	    }
        ...
        $query = "SELECT
                ...
                AND barNumberCaseNumber_tbl.barNumber = $barNumber";
	 ...
	 * @author Scott Brenner
	 **/
	protected function updateNACIntoDb( $array ){
	    if ( $this->verbose ) echo  __METHOD__ . "\n";
        include("passwords/todayspo_MyCourtDates.php");
        // Connect to the db
        try
        ...
        $values = "\"" . $values . "\"";
        $query = "INSERT INTO NAC_tbl ( $columns ) VALUES ( $values ) ON DUPLICATE KEY UPDATE active = \"" . $array[ "active" ] . "\"";
	...
	protected function isScheduleStale( &$attorneyId ){
	    ...
	    // Query DB to get vintage of bar nubmer based schedule.
        if ( $this->verbose ) echo  __METHOD__ . "\n";
        include("passwords/todayspo_MyCourtDates.php");
        ...
        $query = "  SELECT  addOnBarNumber, MAX( vintage ) AS vintage
                    FROM    addOnBarNumber_tbl
                    WHERE   addOnBarNumber = \"$attorneyId\"";
        ...
        if ( $row["vintage"] < date( "Y-m-d H:i:s" ) ) { return true; }
